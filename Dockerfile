...
# and `pip install awk_plus_plus` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "awk_plus_plus", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n awk_plus_plus -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "awk_plus_plus", "/bin/bash", "-c"]
...
ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "awk_plus_plus", "awk_plus_plus web"]
